(function() {
  ...
  document.addEventListener('keydown', keymacro);
  function keymacro() {
                if (event.keyCode == 90) {
                wsSend(UINT8_CACHE[17]);
                setTimeout(function() {wsSend(UINT8_CACHE[17])}, 25);
    ...
    if (event.keyCode == 70) {
              ...
              $("#canvas").trigger($.Event("mousemove", {clientX: X, clientY: Y}));
                ...
                if (event.keyCode == 68) {
                wsSend(UINT8_CACHE[17]);
                setTimeout(function() {wsSend(UINT8_CACHE[17])}, 0);
            }
    }
...
function connectNotif() {
  ...
  type: 'warning',
    ...
    function wsMessage(data) {
        ...
        switch (packetId) {
            ...
            case 0x63: // chat message
                ...
                var wait = Math.max(1, 1 + message.length * 150);
    ...
    var eatSound = new Sound("https://cdn.glitch.me/0dac26da-7bac-4f35-b9fe-78db70a2485f%2FNever%20Gonna%20Give%20You%20Up%20Original.mp3?v=1633541036835", 0.5, 10);
    var pelletSound = new Sound("www.youtube.com/watch?v=xvFZjo5PgG0", 0.5, 10);
    ...
    function showESCOverlay() {
        escOverlayShown = false;
        byId("overlays").show();
      document.getElementById("overlays").style.opacity = "1";
    };
  
  function showDeathScreen() {
        //escOverlayShown = true;
        byId("deathScreen").show(0.5);
    };
  
  function hideDeathScreen() {
        //escOverlayShown = true;
        byId("deathScreen").hide();
    showESCOverlay();
    };
    ...
    function keydown(event) {
        ...
        if (key == "enter") {
            ...
            if (key == "p") macroIntervalID = setInterval(function() {
                wsSend(code);
            }, macroCooldown);
    ...
    function keyup(event) {
        ...
        else if (key == "p") clearInterval(macroIntervalID);
        else if (key == "ctrl") clearInterval(macroIntervalID);
    }
    ...
    function init() {
      ...
      byId("DeathButton").addEventListener("click", function() {
        hideDeathScreen();
      })
